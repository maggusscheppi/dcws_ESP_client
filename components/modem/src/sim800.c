...
static esp_err_t sim800_get_signal_quality(modem_dce_t *dce, uint32_t *rssi, uint32_t *ber)
{
    ...
    ESP_LOGI(DCE_TAG, "inquire signal quality ok");
    return ESP_OK;
err:
    return ESP_FAIL;
}
...
{
    ESP_LOGI(DCE_TAG, "sim800_set_working_mode");
    modem_dte_t *dte = dce->dte;
    switch (mode) {
...
static esp_err_t sim800_power_down(modem_dce_t *dce)
{
    ...
    ESP_LOGI(DCE_TAG, "power down ok");
    return ESP_OK;
err:
    return ESP_FAIL;
}
...
static esp_err_t sim800_get_module_name(sim800_modem_dce_t *sim800_dce)
{
    ...
    ESP_LOGI(DCE_TAG, "get module name ok");
    return ESP_OK;
err:
    return ESP_FAIL;
}
...
static esp_err_t sim800_get_imei_number(sim800_modem_dce_t *sim800_dce)
{
    ...
    ESP_LOGI(DCE_TAG, "get imei number ok");
    return ESP_OK;
err:
    return ESP_FAIL;
}
...
static esp_err_t sim800_get_imsi_number(sim800_modem_dce_t *sim800_dce)
{
    ...
    ESP_LOGI(DCE_TAG, "get imsi number ok");
    return ESP_OK;
err:
    return ESP_FAIL;
}
...
/**
 * @power ON sequence : handler that discards the glitches
 */
static esp_err_t sim800_handle_power_ON(modem_dce_t *dce, const char *line)		//MaGe
{
    ESP_LOGI(DCE_TAG, "Glitch from Modem: %s", line);
    return ESP_OK;
}

static esp_err_t sim800_power_ON(sim800_modem_dce_t *sim800_dce)				//MaGe
{
	modem_dte_t *dte = sim800_dce->parent.dte;
    sim800_dce->parent.handle_line = sim800_handle_power_ON;
	...
	gpio_pad_select_gpio(MODEM_RST);
	gpio_set_direction(MODEM_RST, GPIO_MODE_OUTPUT);

	gpio_pad_select_gpio(MODEM_PWKEY);
	...
	// restart module : needed in case of watchdog
	gpio_set_level(MODEM_PWKEY, LOW);
	gpio_set_level(MODEM_RST, HIGH);
	gpio_set_level(MODEM_POWER_ON, HIGH);		// Turn off the Modem LDO external power supply
	ESP_LOGI(DCE_TAG, "Module GSM un-powered");
	vTaskDelay(3000/portTICK_PERIOD_MS);			// must be between 0,5 and 1s
...
modem_dce_t *sim800_init(modem_dte_t *dte)
{
	...
	/* Turn modem Power ON */
    DCE_CHECK(sim800_power_ON(sim800_dce) == ESP_OK, "Power ON failed", err_io);
	
    /* Sync between DTE and DCE */
	ESP_LOGI(DCE_TAG, "Sync between DTE and DCE");
reinit_dce_sync:
    DCE_CHECK(esp_modem_dce_sync(&(sim800_dce->parent)) == ESP_OK, "sync failed", reinit_dce_sync);
	
    /* Close echo */
	ESP_LOGI(DCE_TAG, "Close echo");
...
reinit_imsi_number:
    ...
    DCE_CHECK(sim800_get_imsi_number(sim800_dce) == ESP_OK, "get imsi failed", reinit_imsi_number);
    /* Get operator name */
